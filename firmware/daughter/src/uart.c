...
void init_UART(bool alternate) {
	if (alternate == true) {
		// select alternate pin position for the output.
...
void UART_push_frame(const struct Frame *f) {
	...
	UART_putc(0x55);
	uint8_t crc = 0;
	for (uint8_t i = 0; i < sizeof(struct Frame); ++i) {
		char d = ((const char *)f)[i];
		UART_putc(d);
		crc = crc8_update(crc, d);
	}
	UART_putc(crc);
}
