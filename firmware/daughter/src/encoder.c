...
void Encoder_init(struct Encoder *enc) {
	enc->value      = 0x02;
	enc->_initState = 0x03;
	Debouncer_init(&enc->_qA);
	Debouncer_init(&enc->_qB);
}
...
void Encoder_update(struct Encoder *enc, bool qA, bool qB) {
	if (enc->_initState != 0) {
		...
		enc->_initState &=
		    ~(((uint8_t)Debouncer_2ms_push(&enc->_qA, qA) << 1) |
		      ((uint8_t)Debouncer_2ms_push(&enc->_qB, qB) << 0));
		enc->_state =
	...
	Debouncer_2ms_push(&enc->_qA, qA);
	Debouncer_2ms_push(&enc->_qB, qB);
